...
CompoundEffect::CompoundEffect(const char* name, RoutingMode mode)
    : Effect(name)
    , routingMode_(mode)
    , passClean_(true)
    , scaleParallel_(false)
...
void CompoundEffect::Process(float* in, float* out, size_t size) {
    ...
    if (routingMode_ == RoutingMode::SERIES) {
        ...
        // Add clean signal if enabled
        if (passClean_) {
            std::memcpy(out, in, size * sizeof(float));
        }
        
        // Process each effect and accumulate
        float gain = scaleParallel_ ? (1.0f / static_cast<float>(effects_.size())) : 1.0f;
        for (Effect* effect : effects_) {
            if (effect) {
...
void CompoundEffect::SetMetronomeEnabled(bool enabled) {
    metronomeEnabled_ = enabled;
    
    // Propagate metronome state to all child effects
    for (Effect* effect : effects_) {
        if (effect) {
            effect->SetMetronomeEnabled(enabled);
        }
    }
}
...
float CompoundEffect::GetTempoPulseBrightness() const {
    ...
    for (const Effect* effect : effects_) {
        if (effect) {
            maxBrightness = std::max(maxBrightness, effect->GetTempoPulseBrightness());
        }
    }
    return maxBrightness;
}

void CompoundEffect::OnSelected() {
    for (Effect* effect : effects_) {
        if (effect) {
            effect->OnSelected();
        }
...
void CompoundEffect::OnDeselected() {
    for (Effect* effect : effects_) {
        if (effect) {
            effect->OnDeselected();
        }
...
void CompoundEffect::SetPassClean(bool passClean) {
    passClean_ = passClean;
}

bool CompoundEffect::GetPassClean() const {
    return passClean_;
}

void CompoundEffect::SetScaleParallel(bool scaleParallel) {
    scaleParallel_ = scaleParallel;
}
