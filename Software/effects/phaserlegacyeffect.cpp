#include "phaserlegacyeffect.h"
#include "../controls.h"

using namespace perspective;
using namespace daisysp;

PhaserLegacyEffect::PhaserLegacyEffect() 
    : Effect("Phaser Legacy") {
}

PhaserLegacyEffect::~PhaserLegacyEffect() {
}

void PhaserLegacyEffect::Init(float sampleRate) {
    ...
    AddParameter(new PotentiometerParameter("K3 Rate", 0.01f, 10.0f, 0.3f, PotCurve::LOG, KNOB_3_IDX));
    AddParameter(new PotentiometerParameter("K4 Feedback", 0.0f, 0.95f, 0.7f, PotCurve::LIN, KNOB_4_IDX));
    AddParameter(new EncoderParameter("E1 Poles", 1.0f, 8.0f, 4.0f, 1.0f, ENCODER_1_IDX));
    
    // Set default phaser parameters
    Update();
}

void PhaserLegacyEffect::Process(float* in, float* out, size_t size) {
    if (!enabled_) {
...
void PhaserLegacyEffect::ProcessStereo(float* inL, float* inR, float* outL, float* outR, size_t size) {
    if (!enabled_) {
        // Bypass - pass through dry signal
        for (size_t i = 0; i < size; i++) {
            outL[i] = inL[i];
...
void PhaserLegacyEffect::Update() {
