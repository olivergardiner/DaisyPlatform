...
void ChorusEffect::Init(float sampleRate) {
    ...
    auto* depthParam = new ValueParameter("K2 Depth", 0.0f, 1.0f, 0.65f);
    ...
    auto* rateParam = new ValueParameter("K3 Rate", 0.05f, 3.0f, 0.35f);
    rateParam->BindPotentiometer(MACRO_KNOB_RATE_IDX, PotCurve::LOG);
    rateParam->SetMacroRole(MacroRole::RATE);
    AddParameter(rateParam);

    auto* feedbackParam = new ValueParameter("K4 Feedback", -0.75f, 0.75f, 0.1f);
    ...
    AddParameter(new ValueParameter("Delay ms", 5.0f, 25.0f, 12.0f));
...
void ChorusEffect::Process(const float* in, float* out, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        float delayMs = baseDelayMs + depth * maxSweepMs * mod;
...
void ChorusEffect::ProcessStereo(const float* inL, const float* inR, float* outL, float* outR, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        float delaySamplesL = (baseDelayMs + depth * maxSweepMs * modL) * sampleRate_ * 0.001f;
        float delaySamplesR = (baseDelayMs + depth * maxSweepMs * modR) * sampleRate_ * 0.001f;
        ...
        writeIndex_ = (writeIndex_ + 1) % kMaxDelaySamples;
        lfoPhaseL_ = WrapPhase(lfoPhaseL_ + phaseInc);
        lfoPhaseR_ = WrapPhase(lfoPhaseR_ + phaseInc * 1.017f);
    }
}
