#include "flangerlegacyeffect.h"
#include "../controls.h"

using namespace perspective;
using namespace daisysp;

FlangerLegacyEffect::FlangerLegacyEffect()
    : Effect("Flanger Legacy") {
}

FlangerLegacyEffect::~FlangerLegacyEffect() {
}

void FlangerLegacyEffect::Init(float sampleRate) {
    ...
    AddParameter(new PotentiometerParameter("K3 Rate", 0.05f, 1.0f, 0.3f, PotCurve::LOG, KNOB_3_IDX));
    AddParameter(new PotentiometerParameter("K4 Feedback", 0.0f, 0.95f, 0.5f, PotCurve::LIN, KNOB_4_IDX));
    
    // Set default flanger parameters
    Update();
}

void FlangerLegacyEffect::Process(float* in, float* out, size_t size) {
    if (!enabled_) {
...
void FlangerLegacyEffect::ProcessStereo(float* inL, float* inR, float* outL, float* outR, size_t size) {
    if (!enabled_) {
        // Bypass - pass through dry signal
        for (size_t i = 0; i < size; i++) {
            outL[i] = inL[i];
...
void FlangerLegacyEffect::Update() {
