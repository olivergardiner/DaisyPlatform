...
void UIEventHandler::DispatchEventToListeners(const UIEvent& event) {
    //Hardware::PrintLine("Dispatching event: type=%d, source=%p, controlIndex=%d, value=%d", event.type, event.source, event.controlIndex, event.value);
    for (const auto& listener : listeners_) {
        ...
        if (listener.eventType == event.type) {
            ...
            if (sourceMatches && indexMatches) {
                //Hardware::PrintLine("Dispatching event to listener: eventType=%d, source=%p, controlIndex=%d", event.type, event.source, event.controlIndex);
